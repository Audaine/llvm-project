...
public:
  ...
  /// Marks all loans sharing the same access path as the moved origin as
  /// potentially moved. Searches backward through predecessor facts to find
  /// all IssueFacts with matching access paths.
  Lattice transfer(Lattice In, const MovedOriginFact &F) {
    ...
    // Find all predecessor IssueFacts with matching AccessPath to mark loans
    // created earlier as potentially moved, not just loans at the move point.
